...
public class Farming {
	...
	private Plant[] plants = new Plant[60];
	private GrassyPatch[] patches = new GrassyPatch[60];
		...
		switch (closest) {
			...
			case FARMING_GUILD_HERB:
				config = (config(FarmingPatches.FARMING_GUILD_FLOWER) << 24)
						+ (config(FarmingPatches.FARMING_GUILD_TREE) << 16)//works
						+ (config(FarmingPatches.FARMING_GUILD_CACTUS) << 8)
						+ (config(FarmingPatches.FARMING_GUILD_HERB));//works
				configv2 = 1057;
				break;
			case LUMBRIDGE_TREE:
					...
					if (plant.seed == seed) {
						if (player.getPA().getLevelForXP(player.playerXP[Skill.FARMING.getId()]) >= plant.level) {
							...
							if(!player.getItems().playerHasItem(seed, plant.seedAmount)){
								player.sendMessage("You need "+plant.seedAmount+" of "+ ItemDef.forId(seed).getName().toLowerCase()+" to plant this.");
								return false;
							}
							...
							if (player.getItems().playerHasItem(sapling ? 5325 : 5343, 1)) {
								...
								player.sendMessage("You plant the "+ ItemDef.forId(plant.seed).getName().toLowerCase()+" in the dirt.");
								player.getItems().deleteItem(seed, plant.seedAmount);
								if(ItemDef.forId(plant.seed).getName().toLowerCase().contains("sapling")){
